import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
import gymnasium as gym
import sys
sys.path.insert(0, './problem_w_unobservable_events')
import uo_problem_env
from str_generator import StringGenerator


PHI_2={
    (1, 'x'):0,
    (2, 'x'):1,
    (3, 'x'):2,
    (4, 'x'):3,
    (5, 'x'):4,
    (6, 'x'):5,
    (7, 'x'):6,
    (8, 'x'):7,
    (9, 'x'):8,
    (10,'x'):9,
    (11,'x'):10,
    (12,'x'):11,
    (13,'x'):12,
    (14,'x'):13,
    (15,'x'):14,
    (16,'x'):15,
    (17,'x'):16,
    (19,'x'):17,
    (21,'x'):18,
    (-1,'x'):19,
    (1, 'y'):20,
    (2, 'y'):21,
    (3, 'y'):22,
    (4, 'y'):23,
    (5, 'y'):24,
    (6, 'y'):25,
    (7, 'y'):26,
    (8, 'y'):27,
    (9, 'y'):28,
    (10,'y'):29,
    (11,'y'):30,
    (12,'y'):31,
    (13,'y'):32,
    (14,'y'):33,
    (15,'y'):34,
    (16,'y'):35,
    (17,'y'):36,
    (19,'y'):37,
    (21,'y'):38,
    (-1,'y'):39,
    (1, 'z'):40,
    (2, 'z'):41,
    (3, 'z'):42,
    (4, 'z'):43,
    (5, 'z'):44,
    (6, 'z'):45,
    (7, 'z'):46,
    (8, 'z'):47,
    (9, 'z'):48,
    (10,'z'):49,
    (11,'z'):50,
    (12,'z'):51,
    (13,'z'):52,
    (14,'z'):53,
    (15,'z'):54,
    (16,'z'):55,
    (17,'z'):56,
    (19,'z'):57,
    (21,'z'):58,
    (-1,'z'):59,
    (1, 's'):60,
    (2, 's'):61,
    (3, 's'):62,
    (4, 's'):63,
    (5, 's'):64,
    (6, 's'):65,
    (7, 's'):66,
    (8, 's'):67,
    (9, 's'):68,
    (10,'s'):69,
    (11,'s'):70,
    (12,'s'):71,
    (13,'s'):72,
    (14,'s'):73,
    (15,'s'):74,
    (16,'s'):75,
    (17,'s'):76,
    (19,'s'):77,
    (21,'s'):78,
    (-1,'s'):79,
    (1, 't'):80,
    (2, 't'):81,
    (3, 't'):82,
    (4, 't'):83,
    (5, 't'):84,
    (6, 't'):85,
    (7, 't'):86,
    (8, 't'):87,
    (9, 't'):88,
    (10,'t'):89,
    (11,'t'):90,
    (12,'t'):91,
    (13,'t'):92,
    (14,'t'):93,
    (15,'t'):94,
    (16,'t'):95,
    (17,'t'):96,
    (19,'t'):97,
    (21,'t'):98,
    (-1,'t'):99,
    (1, 'r'):100,
    (2, 'r'):101,
    (3, 'r'):102,
    (4, 'r'):103,
    (5, 'r'):104,
    (6, 'r'):105,
    (7, 'r'):106,
    (8, 'r'):107,
    (9, 'r'):108,
    (10,'r'):109,
    (11,'r'):110,
    (12,'r'):111,
    (13,'r'):112,
    (14,'r'):113,
    (15,'r'):114,
    (16,'r'):115,
    (17,'r'):116,
    (19,'r'):117,
    (21,'r'):118,
    (-1,'r'):119,
}

successful_protocols = pd.read_csv("problem_w_unobservable_events/simulation_2_successful_protocols.csv")

print(len(successful_protocols["Communication Protocols"].unique()))

communication_dict={}

a1_protocol=[0 for _ in range(40)]
a2_protocol=[0 for _ in range(120)]

for index, row in successful_protocols.iterrows():
    print(f"{index} / {len(successful_protocols)}", end="\r")
    protocol = row["Communication Protocols"].replace("(","").replace(")","").split(", ")
    protocol = [int(x) for x in protocol]
    
    q_1 = protocol[:40]
    q_2 = protocol[40:]
    
    a1_protocol = a1_protocol + np.array(q_1)
    a2_protocol = a2_protocol + np.array(q_2)

a1_protocol = a1_protocol / len(successful_protocols)
a2_protocol = a2_protocol / len(successful_protocols)

print(a1_protocol)
print(a2_protocol)

print("-----HIGH PROBABILITY COMMUNICATIONS-----")
for key in PHI_2.keys():
    if a2_protocol[PHI_2[key]]>0.6:
        print(key, a2_protocol[PHI_2[key]])

print("-----LOW PROBABILITY COMMUNICATIONS-----")

for key in PHI_2.keys():
    if a2_protocol[PHI_2[key]]<=0.6 and a2_protocol[PHI_2[key]]>0:
        print(key, a2_protocol[PHI_2[key]])

        
'''
  m_bottom={
        1:  {1,3},
        2:  {2,4,5,12,20,17},
        3:  {6,21},
        4:  {6,10},
        5:  {2,4,8,13,18},
        6:  {6},
        7:  {7},
        8:  {2,4,14},
        9:  {9},
        10: {10},
        11: {11},
        12: {2,4},
        13: {5,12,17,20},
        14: {14},
        15: {15},
        16: {16},
        17: {8,13,18},
        19: {19},
        21: {21},
        -1: {-1},
    }

(1, 'x') 1.0
(1, 'y') 1.0
(1, 'z') 1.0

(2, 'x') 1.0
(2, 'y') 0.986
(2, 'z') 0.998

(5, 'x') 0.998
(5, 'z') 0.918

(8, 'x') 0.994

(12, 'x') 0.969

(-1, 'x') 1.0
(-1, 'y') 1.0
(-1, 'z') 1.0
(-1, 's') 1.0
(-1, 't') 1.0
(-1, 'r') 1.0

-------------------------------

(3, 's') 0.159

(13, 'x') 0.531
(13, 'y') 0.491
(13, 'z') 0.49

(16, 'x') 0.193
(16, 'y') 0.467
(16, 'z') 0.491

(17, 'x') 0.398
(17, 'z') 0.48

(19, 't') 0.312

(21, 's') 0.153


(8, 'z') 0.027
(17, 'y') 0.015
(14, 'z') 0.004
(15, 'r') 0.092
(5, 'y') 0.082

'''